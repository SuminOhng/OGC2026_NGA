"""Compute a geometry-aware relaxed CP-SAT lower bound for OGC obj1.

This is an analysis tool, not submitted solver logic.  It starts from the
bay-level cumulative-area relaxation in ``lower_bound_cp_sat.py`` and adds only
pairwise constraints that can be proven safely:

If two blocks assigned to the same bay cannot coexist in any integer
orientation/position pair while allowing either entry/exit order, then their
intervals cannot overlap in that bay.

Pairs whose coexistence check would be too expensive are left unconstrained.
That weakens the bound but keeps it conservative.
"""
...
def main() -> None:
    ...
    result = geometry_pair_tardiness_lb(
        prob_info,
...
def geometry_pair_tardiness_lb(
    ...
    *,
    ...
    incompatible, unknown_tests = _proven_incompatible_pairs(
        prob_info,
        geometry,
    ...
    pair_constraints = 0
    for bay_id, left_id, right_id in incompatible:
        ...
        if left_presence is None or right_presence is None:
            continue
        before = model.NewBoolVar(f"before_{left_id}_{right_id}_b{bay_id}")
        model.Add(ends[left_id] <= starts[right_id]).OnlyEnforceIf([left_presence, right_presence, before])
        model.Add(ends[right_id] <= starts[left_id]).OnlyEnforceIf([left_presence, right_presence, before.Not()])
        pair_constraints += 1
    ...
    return {
        ...
        "geometry_cp_obj1_bound": cp_bound,
        "geometry_cp_obj1_incumbent": cp_incumbent,
        "combined_obj1_lb": combined_obj1_lb,
        "combined_weighted_lb": combined_weighted_lb,
        "proven_incompatible_bay_pairs": len(incompatible),
...
def _proven_incompatible_pairs(
    prob_info: dict[str, Any],
        ...
        for left_id in range(len(blocks)):
            ...
            for right_id in range(left_id + 1, len(blocks)):
                ...
                if not right_options:
                    continue
                can_overlap, complete = _pair_can_overlap_in_bay(
                    prob_info,
                    bay_id,
                    left_options,
                    right_options,
                ...
                if not complete:
                    ...
                    continue
                if not can_overlap:
                    incompatible.append((bay_id, left_id, right_id))
    return incompatible, unknown_tests
    ...
    *,
    ...
    for left in left_options:
        for right in right_options:
            ...
            # Co-presence can be FIFO or nested.  A valid relaxation must accept
            # any entry order combined with any feasible first-exit order.
            ...
            if left_then_right_entry and (left_first_exit or right_first_exit):
                return True, True
            if right_then_left_entry and (left_first_exit or right_first_exit):
                return True, True
    return False, True
